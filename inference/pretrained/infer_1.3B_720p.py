...
from inference_img.utils import load_image_sequence
...
for i in range(5):
    ...
    pisco_video_mask = load_image_sequence(os.path.join(base_path, "mask"), height=720, width=1280, length=49)
    pisco_video_mask.set_length(49)
    
    pisco_reference_video = load_image_sequence(os.path.join(base_path, "video_masked"), height=720, width=1280, length=49)
    pisco_reference_video.set_length(49)
    
    pisco_reference_depth = load_image_sequence(os.path.join(base_path, "video_depth_masked"), height=720, width=1280, length=49)

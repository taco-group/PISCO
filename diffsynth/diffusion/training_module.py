...
class DiffusionTrainingModule(torch.nn.Module):
    ...
    def export_trainable_state_dict(self, state_dict, remove_prefix=None):
        ...
        # ---- normalize remove_prefix to a tuple of prefixes ----
        if isinstance(remove_prefix, (list, tuple)):
            prefixes = [p.strip() for p in remove_prefix if isinstance(p, str) and p.strip()]
        elif isinstance(remove_prefix, str):
            # support comma-separated string like "pipe.xgv2.,pipe.dit."
            prefixes = [p.strip() for p in remove_prefix.split(",") if p.strip()]
        else:
            raise TypeError(f"remove_prefix must be str/list/tuple/None, got {type(remove_prefix)}")

        prefixes = tuple(prefixes)

        # ---- remove at most one matching prefix per key ----
        state_dict_ = {}
    ...
    def parse_model_configs(self, model_paths, model_id_with_origin_paths, fp8_models=None, offload_models=None, device="cpu"):
        ...
        if model_paths is not None:
            try:
                ...
                vram_config = self.parse_vram_config(
                    fp8=model_paths in fp8_models,
                    offload=model_paths in offload_models,
                    device=device
                )
                model_configs.append(ModelConfig(path=model_paths, **vram_config))
    ...
    def switch_pipe_to_training_mode(
        ...
        if lora_base_model is not None and not task.endswith(":data_process"):
            lora_base_list = lora_base_model.split(",")
            for lora_base in lora_base_list:
                if (not hasattr(pipe, lora_base)) or getattr(pipe, lora_base) is None:
                    print(f"No {lora_base} models in the pipeline. We cannot patch LoRA on the model. If this occurs during the data processing stage, it is normal.")
                    return
                model = self.add_lora_to_model(
                    getattr(pipe, lora_base),

...
class VaceWanModel(torch.nn.Module):
    ...
    def forward(
        ...
        # vace_context: torch.Size([1, 96, 13, 60, 104])
        c = [self.vace_patch_embedding(u.unsqueeze(0)) for u in vace_context] # List of torch.Size([1, 1536, 13, 30, 52]) of length 1
        c = [u.flatten(2).transpose(1, 2) for u in c] # List of torch.Size([1, 20280, 1536]) of length 1
        c = torch.cat([

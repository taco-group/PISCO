...
from ..models.wan_video_pisco import PISCOWanModel
...
class WanVideoPipeline(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.bfloat16):
        ...
        self.pisco: PISCOWanModel = None
        self.pisco2: PISCOWanModel = None
        self.vap: MotWanModel = None
        self.animate_adapter: WanAnimateAdapter = None
        self.audio_encoder: WanS2VAudioEncoder = None
        self.in_iteration_models = ("dit", "motion_controller", "vace", "pisco", "animate_adapter", "vap")
        self.in_iteration_models_2 = ("dit2", "motion_controller", "vace2", "pisco2", "animate_adapter", "vap")
        self.units = [
    ...
    def from_pretrained(
        ...
        pisco = model_pool.fetch_model("wan_video_pisco", index=2)
        if isinstance(pisco, list):
            pipe.pisco, pipe.pisco2 = pisco
        else:
            pipe.pisco = pisco    
    ...
    def __call__(
        ...
        # PISCO
        pisco_video: Optional[list[Image.Image]] = None,
        pisco_video_mask: Optional[list[Image.Image]] = None,
        pisco_reference_video: Optional[list[Image.Image]] = None,
        pisco_depth: Optional[list[Image.Image]] = None,
        pisco_reference_depth: Optional[list[Image.Image]] = None,
        pisco_scale: Optional[float] = 1.0,
        # Animate
        ...
        inputs_shared = {
            ...
            "pisco_video": pisco_video, "pisco_video_mask": pisco_video_mask, "pisco_reference_video": pisco_reference_video, 
            "pisco_depth": pisco_depth, "pisco_reference_depth": pisco_reference_depth, "pisco_scale": pisco_scale,
...
class WanVideoUnit_NoiseInitializer(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=(
                "height", "width", "num_frames", "seed", "rand_device", 
    ...
    def _get_latent_len(self, data):
        """
        Helper function: Calculate Latent frames for input data
        WanVideo VAE time compression ratio is typically 4
        """
        if data is None:
            return 0
        
        # Handle tuple format (video, mask)
        if isinstance(data, tuple):
            data = data[0]
            
        # Get number of frames
        if isinstance(data, (list, VideoData)):
            f = len(data)
        else:
            f = 1 # Single image or element treated as 1 frame
            
        # Calculate latent length: (f-1)//4 + 1
        return (f - 1) // 4 + 1

    def process(self, pipe, height, width, num_frames, seed, rand_device, 
                vace_reference_image):
        
        # 1. Calculate target video base Latent length
        target_len = (num_frames - 1) // 4 + 1
        
        # 2. Calculate extra length needed for Reference
        # Note: Keeping original logic (accumulate length if input exists).
        
        ref_len = 0
        ref_len += self._get_latent_len(vace_reference_image)
        


        # 3. Generate noise
        # Generate noise for total length (Target + Reference)
        total_length = target_len + ref_len
    ...
    def process(
        ...
        if vace_video is not None or vace_video_mask is not None or vace_reference_image is not None:
            ...
            if vace_video is None:
                ...
                vace_video = pipe.preprocess_video(vace_video) # torch.Size([1, 3, 49, 480, 832])
            if vace_video_mask is None:
                vace_video_mask = torch.ones_like(vace_video)
            else:
                vace_video_mask = pipe.preprocess_video(vace_video_mask, min_value=0, max_value=1) # torch.Size([1, 3, 49, 480, 832])
            inactive = vace_video * (1 - vace_video_mask) + 0 * vace_video_mask
            reactive = vace_video * vace_video_mask + 0 * (1 - vace_video_mask)
            inactive = pipe.vae.encode(inactive, device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride).to(dtype=pipe.torch_dtype, device=pipe.device) # torch.Size([1, 16, 13, 60, 104])
            reactive = pipe.vae.encode(reactive, device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride).to(dtype=pipe.torch_dtype, device=pipe.device) # torch.Size([1, 16, 13, 60, 104])
            vace_video_latents = torch.concat((inactive, reactive), dim=1) # torch.Size([1, 32, 13, 60, 104])
            
            vace_mask_latents = rearrange(vace_video_mask[0,0], "T (H P) (W Q) -> 1 (P Q) T H W", P=8, Q=8) # torch.Size([1, 64, 49, 60, 104])
            vace_mask_latents = torch.nn.functional.interpolate(vace_mask_latents, size=((vace_mask_latents.shape[2] + 3) // 4, vace_mask_latents.shape[3], vace_mask_latents.shape[4]), mode='nearest-exact') # torch.Size([1, 64, 13, 60, 104])
            ...
            if vace_reference_image is None:
                ...
                vace_reference_image = pipe.preprocess_video(vace_reference_image) # torch.Size([1, 3, 1, 480, 832])
                ...
                vace_reference_latents = pipe.vae.encode(vace_reference_image, device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride).to(dtype=pipe.torch_dtype, device=pipe.device) # torch.Size([1, 16, 1, 60, 104])
                vace_reference_latents = torch.concat((vace_reference_latents, torch.zeros_like(vace_reference_latents)), dim=1) # torch.Size([1, 32, 1, 60, 104])
                vace_reference_latents = [u.unsqueeze(0) for u in vace_reference_latents]

                vace_video_latents = torch.concat((*vace_reference_latents, vace_video_latents), dim=2) # torch.Size([1, 32, 14, 60, 104])
                vace_mask_latents = torch.concat((torch.zeros_like(vace_mask_latents[:, :, :f]), vace_mask_latents), dim=2) # torch.Size([1, 64, 14, 60, 104])
            
            vace_context = torch.concat((vace_video_latents, vace_mask_latents), dim=1) # torch.Size([1, 96, 14, 60, 104])
...
class WanVideoUnit_PISCO(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("pisco_video", "pisco_reference_video", "pisco_video_mask", "pisco_depth", "pisco_reference_depth", "pisco_scale", "tiled", "tile_size", "tile_stride"),
            output_params=("pisco_context", "pisco_scale"),
            onload_model_names=("vae",)
        )
    ...
    def process(
        self,
        pipe: WanVideoPipeline,
        pisco_video, pisco_reference_video, pisco_video_mask, pisco_depth, pisco_reference_depth, pisco_scale,
        tiled, tile_size, tile_stride
    ):

        enable_detailed_mask_channel = True 

        def interpolate_fill(video_tensor, mask_1d):
            """
            video_tensor: [B, C, T, H, W]
            mask_1d: [T] (0=missing, 1=valid)
            Returns: Nearest-filled video
            """
            if mask_1d.sum() == 0 or mask_1d.sum() == mask_1d.shape[0]:
                return video_tensor
            
            B, C, T, H, W = video_tensor.shape
            
            # Find all valid indices [N_valid]
            valid_indices = torch.where(mask_1d > 0.5)[0] # e.g. [0, 5, 10]
            
            # Create a [T] tensor, storing the nearest valid index for each position
            # Method: Calculate distance between all t and valid_indices, take index of min value
            # [T, 1] - [1, N_valid] -> [T, N_valid]
            arange_t = torch.arange(T, device=video_tensor.device).unsqueeze(1)
            dists = torch.abs(arange_t - valid_indices.unsqueeze(0))
            nearest_idx_in_valid = torch.argmin(dists, dim=1) # [T] -> index in valid_indices
            nearest_frame_indices = valid_indices[nearest_idx_in_valid] # [T] -> actual frame index
            
            # Use gather to fill
            # gather requires index dimensions to match: [B, C, T, H, W]
            indices_expanded = nearest_frame_indices.view(1, 1, T, 1, 1).expand(B, C, T, H, W)
            out = torch.gather(video_tensor, 2, indices_expanded)
            
            return out
      

        if pisco_video is not None or pisco_reference_video is not None or pisco_reference_depth is not None:
            # [UNCHANGED] Unpack & Preprocess
            assert pisco_reference_video is not None and pisco_reference_depth is not None
            pisco_reference_video, pisco_temporal_mask = pisco_reference_video
            pisco_depth, _ = pisco_depth
            pisco_reference_depth, _ = pisco_reference_depth
            pisco_video_mask, _ = pisco_video_mask
            
            pipe.load_models_to_device(["vae"])

            pisco_video = pipe.preprocess_video(pisco_video) 
            pisco_reference_video = pipe.preprocess_video(pisco_reference_video)
            pisco_depth = pipe.preprocess_video(pisco_depth)
            pisco_reference_depth = pipe.preprocess_video(pisco_reference_depth)
            pisco_video_mask = pipe.preprocess_video(pisco_video_mask, min_value=0, max_value=1)
            # pisco_reference_video = pisco_reference_video *  pisco_video_mask + pisco_video * (1-pisco_video_mask)

            pisco_temporal_mask = pisco_temporal_mask.to(device=pipe.device)
            pisco_reference_video = interpolate_fill(pisco_reference_video, pisco_temporal_mask)
            pisco_reference_depth = interpolate_fill(pisco_reference_depth, pisco_temporal_mask)

            target_dtype = pipe.torch_dtype
            pisco_video_latents = pipe.vae.encode(pisco_video.to(dtype=target_dtype), device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride).to(dtype=target_dtype, device=pipe.device)
            pisco_reference_video_latents = pipe.vae.encode(pisco_reference_video.to(dtype=target_dtype), device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride).to(dtype=target_dtype, device=pipe.device)
            pisco_depth_latents = pipe.vae.encode(pisco_depth.to(dtype=target_dtype), device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride).to(dtype=target_dtype, device=pipe.device)
            pisco_reference_depth_latents = pipe.vae.encode(pisco_reference_depth.to(dtype=target_dtype), device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride).to(dtype=target_dtype, device=pipe.device)

            vae_stride = 4
            pad_len = (vae_stride - (pisco_temporal_mask.shape[0] % vae_stride)) % vae_stride
            temp_mask_float = pisco_temporal_mask.to(device=pipe.device, dtype=pipe.torch_dtype)
            temp_mask_padded = torch.nn.functional.pad(temp_mask_float, (0, pad_len), value=0)
            
            block_mask = torch.nn.functional.max_pool1d(
                temp_mask_padded.view(1, 1, -1), kernel_size=vae_stride, stride=vae_stride
            ).view(1, 1, -1, 1, 1)

            # Apply mask to the encoded latents
            pisco_reference_video_latents = pisco_reference_video_latents * block_mask
            pisco_reference_depth_latents = pisco_reference_depth_latents * block_mask
            pisco_mask_latents = rearrange(pisco_video_mask[0,0], "T (H P) (W Q) -> 1 (P Q) T H W", P=8, Q=8)
            
            pisco_mask_latents = pisco_mask_latents * pisco_temporal_mask[None, None, :, None, None]

            pad_t = (vae_stride - (pisco_mask_latents.shape[2] % vae_stride)) % vae_stride
            if pad_t > 0:
                pisco_mask_latents = torch.nn.functional.pad(pisco_mask_latents, (0,0, 0,0, 0,pad_t), value=0)
            
            pisco_mask_latents = torch.nn.functional.max_pool3d(
                pisco_mask_latents, kernel_size=(vae_stride, 1, 1), stride=(vae_stride, 1, 1), padding=0
            )
            mask_feature = None
            if enable_detailed_mask_channel:
                B, C, T_latent, H_latent, W_latent = pisco_reference_video_latents.shape
                target_len = T_latent * vae_stride
                pixel_mask_tensor = pisco_temporal_mask.to(dtype=pipe.torch_dtype, device=pipe.device) 
                
                expanded_mask = torch.zeros(target_len, dtype=pipe.torch_dtype, device=pipe.device)
                valid_len = min(pixel_mask_tensor.shape[0], target_len)
                expanded_mask[:valid_len] = pixel_mask_tensor[:valid_len]
                
                mask_feature = expanded_mask.view(T_latent, vae_stride).permute(1, 0)
                mask_feature = mask_feature[None, :, :, None, None].expand(B, -1, -1, H_latent, W_latent)

            concat_list = [
                pisco_video_latents, 
                pisco_reference_video_latents, 
                pisco_depth_latents, 
                pisco_reference_depth_latents,
                pisco_mask_latents
            ]
            
            if mask_feature is not None:
                concat_list.append(mask_feature)

            pisco_context = torch.concat(concat_list, dim=1)
            return {"pisco_context": pisco_context, "pisco_scale": pisco_scale}
        else:
            return {"pisco_context": None, "pisco_scale": pisco_scale}
    ...
    **kwargs,
    ...
    if sliding_window_size is not None and sliding_window_stride is not None:
        model_kwargs = dict(
            ...
            pisco_context=pisco_context,
            pisco_scale=pisco_scale,
            tea_cache=tea_cache,
    ...
    if pisco_context is not None:
        pisco_hints = pisco(
            x, pisco_context, context, t_mod, freqs,
            use_gradient_checkpointing=use_gradient_checkpointing,
            use_gradient_checkpointing_offload=use_gradient_checkpointing_offload
        )
    ...
    if tea_cache_update:
        ...
        for block_id, block in enumerate(dit.blocks):
            ...
            # PISCO
            if pisco_context is not None and block_id in pisco.pisco_layers_mapping:
                current_pisco_hint = pisco_hints[pisco.pisco_layers_mapping[block_id]]
                if use_unified_sequence_parallel and dist.is_initialized() and dist.get_world_size() > 1:
                    current_pisco_hint = torch.chunk(current_pisco_hint, get_sequence_parallel_world_size(), dim=1)[get_sequence_parallel_rank()]
                    current_pisco_hint = torch.nn.functional.pad(current_pisco_hint, (0, 0, 0, chunks[0].shape[1] - current_pisco_hint.shape[1]), value=0)
                x = x + current_pisco_hint * pisco_scale

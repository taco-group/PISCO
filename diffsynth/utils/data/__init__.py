...
def zero_out_image(image, height, width):
    image = np.array(image)
    image_height, image_width, _ = image.shape
    image = Image.fromarray(image * 0).resize((width, height))
    return image


class VideoData:
    def __init__(self, video_file=None, image_folder=None, height=None, width=None, masked_ids=[], **kwargs):
    ...
    def __getitem__(self, item):
        ...
        if item in self.masked_ids:
            frame = zero_out_image(frame, self.height, self.width)
        return frame
